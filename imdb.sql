...
from movie group by month(date_published) order by month(date_published);
...
-- Type your code below:
select
g.genre, count(movi.id) as num_of_movie from genre g
inner join
movie movi
on
g.movie_id = movi.id
...
where genre_count = 1 group by genre_count;
...
(
   select genre,count(movie_id) as movie_count,
	  Rank() 
      over(order by count(movie_id) desc) as genre_rank from genre  
      group by genre)
...
MIN(median_rating) AS min_med_ratings,
MAX(median_rating) AS max_med_ratings
...
SELECT production_company , COUNT(id) AS movie_count , DENSE_RANK() OVER(ORDER BY COUNT(id) DESC) AS production_rank
FROM movie AS mov
INNER JOIN ratings AS rat
ON mov.id = rat.movie_id
...
-- Type your code below:
	...
	SELECT country , sum(total_votes) AS totalVotes
    FROM movie AS mov
    INNER JOIN ratings AS rat
    ON mov.id = rat.movie_id
...
WITH next_date_published_detail
     AS(SELECT d.name_id, name, d.movie_id, duration, rat.avg_rating, total_votes, mov.date_published,
	  Lead(date_published,1) OVER(partition BY d.name_id ORDER BY date_published,movie_id ) AS next_date_published
		FROM director_mapping AS d
			INNER JOIN names AS nam
				ON nam.id = d.name_id
			INNER JOIN movie AS mov
				ON mov.id = d.movie_id
			INNER JOIN ratings AS rat
